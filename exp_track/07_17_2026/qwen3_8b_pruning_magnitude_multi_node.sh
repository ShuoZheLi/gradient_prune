...
# Runtime config
# -----------------------------
RUN_NAME="${RUN_NAME:-qwen3_8b_score_magnitude_math7500}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-${RUN_NAME/_prune_/_}}"
EXPERIMENT_NAME="${EXPERIMENT_NAME/_score_/_}"
...
torchrun_args=(
  ...
  --rdzv-id "score_magnitude_${SLURM_JOB_ID:-manual}"
)
runner_args=(
  -m experiment_runner
  --config "$CONFIG_FILE"
...
# This launcher only writes magnitude score tensors and exits. Magnitude scores
# are weight-only, so no calibration collection, pruning sweep, or evaluation is
# needed.
...
printf ' %q' python "${prep_runner_args[@]}"
printf '\n'
...
if [[ "$PREP_WITH_TORCHRUN" == "1" ]]; then
  ...
  python "${prep_runner_args[@]}"
fi

echo "[prune] magnitude score generation complete: $SCORE_ROOT"
