#!/bin/bash
#SBATCH --job-name=qwen2d5_7b_math_resp_s0d2
...
#SBATCH --time=6:00:00
#SBATCH --output=slurm-%j_qwen2d5_7b_math_resp_s0d2.out
#SBATCH --error=slurm-%j_qwen2d5_7b_math_resp_s0d2.err
...
# Uses WANDA scores generated from the Qwen2.5-Math-7B-Instruct math7500 correct calibration dataset.
...
# Runtime config
# -----------------------------
RUN_NAME="${RUN_NAME:-qwen2d5_7b_math_instruct_wanda_cali_data_math7500_response_analysis}"
...
PRUNING_SPARSITY="${PRUNING_SPARSITY:-0.2}"
BASE_MODEL_ID="${BASE_MODEL_ID:-qwen2d5_7b_math_instruct_dense}"
PRUNED_MODEL_ID="${PRUNED_MODEL_ID:-qwen2d5_7b_math_instruct_wanda_s${PRUNING_SPARSITY}}"
DATASET_PATH="${DATASET_PATH:-/work2/09576/shuozhe/saved_dataset/MetaMathQA-math-500/test.parquet}"
CALIBRATION_DATA_LABEL="${CALIBRATION_DATA_LABEL:-Qwen2.5-Math-7B-Instruct math7500 correct calibration dataset}"
SCORE_GENERATION_RUN_ID="${SCORE_GENERATION_RUN_ID:-qwen2d5_7b_instruct_prune_wanda_math7500}"
SCORE_ROOT="${SCORE_ROOT:-/scratch/09576/shuozhe/gradient_prune/results/qwen2d5_7b_math_instruct_wanda_math7500/scores}"
...
PY
  ...
  "$python_bin" - <<'LOG_ACCURACY_PY' "$combined_dir/aggregate_metrics.csv" "$K"
from pathlib import Path
import sys
import pandas as pd
...
k = sys.argv[2]
if not path.is_file():
    raise SystemExit(f"Missing aggregate metrics file: {path}")
df = pd.read_csv(path)
metric_cols = [col for col in ["pass_at_1", "pass_at_k", "avg_at_k", "accuracy"] if col in df.columns]
display_name = {"pass_at_1": "pass@1", "pass_at_k": f"pass@{k}", "avg_at_k": f"avg@{k}"}
if not metric_cols:
    print(f"No pass/avg accuracy metrics found in {path}")
else:
    print(f"Accuracy summary from {path} (K={k}; pass@{k}/avg@{k} use all generated samples):")
    for _, row in df.iterrows():
        pieces = [f"model_id={row.get('model_id', '')}"]
