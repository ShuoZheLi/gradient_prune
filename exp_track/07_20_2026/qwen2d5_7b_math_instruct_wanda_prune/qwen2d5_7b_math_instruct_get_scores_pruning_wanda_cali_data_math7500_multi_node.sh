#!/bin/bash
#SBATCH --job-name=qwen2d5_7b_math_instruct_wanda_scores
...
#SBATCH --time=0:30:00
#SBATCH --output=slurm-%j_qwen2d5_7b_math_instruct_wanda_scores.out
#SBATCH --error=slurm-%j_qwen2d5_7b_math_instruct_wanda_scores.err

set -euo pipefail

# Self-contained multi-node WANDA score launcher for Qwen2.5-Math-7B-Instruct.
# By default this runs only the prep/scoring phase and writes reusable score
# tensors plus metadata under $SCORE_ROOT.
...
# Runtime config
# -----------------------------
RUN_NAME="${RUN_NAME:-qwen2d5_7b_math_instruct_prune_wanda_math7500}"
...
MODEL_PATH="${MODEL_PATH:-/work2/09576/shuozhe/saved_model/Qwen2.5-Math-7B-Instruct}"
CALIBRATION_PATH="${CALIBRATION_PATH:-/work/09576/shuozhe/gradient_prune/saved_calibration_dataset/qwen2d5-math-7b-instruct_math7500_correct/qwen2d5-math-7b-instruct_math7500_correct.parquet}"
...
if [[ "$DRY_RUN" != "1" && ! -e "$CALIBRATION_PATH" ]]; then
  ...
  echo "Set CALIBRATION_PATH=/path/to/calibration.parquet visible on all compute nodes." >&2
  exit 2
fi
if [[ "$LOAD_SCORES" == "true" && "$DRY_RUN" != "1" && ! -f "$SCORE_ROOT/metadata.json" ]]; then
  echo "Score root does not contain saved scores: $SCORE_ROOT" >&2
  echo "Set SCORE_ROOT=/path/to/saved/scores or set LOAD_SCORES=false to generate scores." >&2
  exit 3
fi
...
python3 - "$CONFIG_FILE" "$RESULTS_ROOT" "$MODEL_PATH" "$SCORE_ROOT" "$EXPERIMENT_NAME" "$LOAD_SCORES" "$CALIBRATION_PATH" <<'CONFIG_PATH_PY'
import sys
...
text = text.replace("__CALIBRATION_PATH__", calibration_path)
...
torchrun_args=(
  ...
  --rdzv-id "qwen2d5_7b_math_instruct_wanda_scores_${SLURM_JOB_ID:-manual}"
)
prep_runner_args=(
  -m experiment_runner
  --config "$CONFIG_FILE"
...
# This launcher is for generating reusable WANDA score files. Leave SCORE_ONLY=1
# for score generation only; set SCORE_ONLY=0 to continue into pruning/eval.
...
echo "[prune] calibration_path=$CALIBRATION_PATH"
...
printf '\n'
echo "[prune] prep_with_torchrun=$PREP_WITH_TORCHRUN score_only=$SCORE_ONLY"
