#!/bin/bash
#SBATCH --job-name=qwen3_4b_inst_align_s0d2
...
#SBATCH --output=slurm-%j_qwen3_4b_inst_align_s0d2.out
#SBATCH --error=slurm-%j_qwen3_4b_inst_align_s0d2.err
...
# Runtime config
# -----------------------------
RUN_NAME="${RUN_NAME:-qwen3_4b_instruct_teacher_student_alignment_sparsity_0d2}"
...
MODEL_ID="${MODEL_ID:-qwen3_4b_instruct_teacher_wanda_sparsity_0d2}"
PRUNING_SPARSITY="${PRUNING_SPARSITY:-0.2}"
INPUT_JSONL="${INPUT_JSONL:-/scratch/09576/shuozhe/gradient_prune/results/response_analysis/qwen3_8b_wanda_response_analysis/runs/qwen3_8b_wanda_response_analysis_839836_20260717_181828/combined/generations.jsonl}"
...
if [[ "$PRUNING_SPARSITY" != "0.2" ]]; then
  echo "This launcher is labeled for sparsity 0.2; got PRUNING_SPARSITY=$PRUNING_SPARSITY" >&2
  exit 1
fi
