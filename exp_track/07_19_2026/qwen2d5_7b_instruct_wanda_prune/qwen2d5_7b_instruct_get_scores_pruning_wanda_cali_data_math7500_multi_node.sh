#!/bin/bash
#SBATCH --job-name=qwen2d5_7b_wanda_scores
...
# Self-contained multi-node WANDA score launcher for Qwen2.5-7B-Instruct.
...
# Runtime config
# -----------------------------
RUN_NAME="${RUN_NAME:-qwen2d5_7b_instruct_prune_wanda_math7500}"
...
CALIBRATION_PATH="${CALIBRATION_PATH:-/work/09576/shuozhe/gradient_prune/saved_calibration_dataset/qwen2d5-7b-instruct_math7500_correct/qwen2d5-7b-instruct_math7500_correct.parquet}"
...
torchrun_args=(
  ...
  --rdzv-id "qwen2d5_wanda_scores_${SLURM_JOB_ID:-manual}"
)
prep_runner_args=(
  -m experiment_runner
  --config "$CONFIG_FILE"
