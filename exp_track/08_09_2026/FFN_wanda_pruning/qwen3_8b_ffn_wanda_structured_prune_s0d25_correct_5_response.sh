#!/bin/bash
#SBATCH --job-name=qwen3_8b_ffn_s0d25_5resp
...
#SBATCH --time=3:00:00
#SBATCH --output=slurm-%j_qwen3_8b_ffn_wanda_s0d25_correct_5_response.out
#SBATCH --error=slurm-%j_qwen3_8b_ffn_wanda_s0d25_correct_5_response.err
...
DEFAULT_SPARSITY="${SPARSITY:-0.25}"
...
if [[ -n "$DEFAULT_TARGET_INTERMEDIATE_SIZE" ]]; then
  ...
  PRUNE_TAG="${SPARSITY_TAG}_correct_5_response"
fi
...
CALIBRATION_PATH="${CALIBRATION_PATH:-/work/09576/shuozhe/gradient_prune/saved_calibration_dataset/qwen3-8b-instruct_math7500_correct_5_response}"
