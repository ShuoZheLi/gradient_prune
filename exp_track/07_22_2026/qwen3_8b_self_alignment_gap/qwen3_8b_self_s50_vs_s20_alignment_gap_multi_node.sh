#!/bin/bash
#SBATCH --job-name=qwen3_8b_s50_vs_s20
...
#SBATCH --output=slurm-%j_qwen3_8b_s50_vs_s20.out
#SBATCH --error=slurm-%j_qwen3_8b_s50_vs_s20.err

set -euo pipefail

# Multi-node sparse-vs-sparse 8B self-alignment launcher, scoring s20 generations under s50.
# Scores 8B WANDA s20 generations under the WANDA s50 8B model, sharding JSONL rows
# across allocated Slurm nodes and merging per-example/aggregate alignment outputs.
...
# Runtime config
# -----------------------------
STUDENT_SPARSITY_LABEL="${STUDENT_SPARSITY_LABEL:-50}"
TEACHER_SPARSITY_LABEL="${TEACHER_SPARSITY_LABEL:-20}"
RUN_NAME="${RUN_NAME:-qwen3_8b_self_alignment_gap_s${STUDENT_SPARSITY_LABEL}_vs_s${TEACHER_SPARSITY_LABEL}}"
...
MODEL_ID="${MODEL_ID:-qwen3_8b_wanda_s0.50_scoring_s0.20_generations}"
PRUNING_SPARSITY="${PRUNING_SPARSITY:-0.50}"
SCORE_ROOT="${SCORE_ROOT:-$RESULTS_BASE/qwen3_8b_wanda_math7500/scores}"
INPUT_JSONL="${INPUT_JSONL:-${SPARSE_8B_GENERATIONS_JSONL:-/scratch/09576/shuozhe/gradient_prune/results/response_analysis/qwen3_8b_wanda_response_analysis/runs/qwen3_8b_wanda_response_analysis_839836_20260717_181828/combined/generations.jsonl}}"
...
if [[ ! -f "$INPUT_JSONL" ]]; then
  ...
  echo "Set INPUT_JSONL=/path/to/sparse_8b/generations.jsonl or SPARSE_8B_GENERATIONS_JSONL=/path/to/sparse_8b/generations.jsonl." >&2
  exit 1
fi
...
if [[ ! -f "$SCORE_ROOT/metadata.json" ]]; then
  ...
  echo "Set SCORE_ROOT=/path/to/saved/scores." >&2
  exit 1
fi
...
build_alignment_args() {
  ...
  local args=(
    ...
    --model_id "$MODEL_ID"
    --pruning_sparsity "$PRUNING_SPARSITY"
    --prune_score_dir "$SCORE_ROOT"
    --prune_granularity "$PRUNE_GRANULARITY"
    --output_dir "$shard_output"
    --dtype "$DTYPE"
    --batch_size "$BATCH_SIZE"
  )
  if [[ -n "$PRUNE_SCORE_KEY" ]]; then args+=(--prune_score_key "$PRUNE_SCORE_KEY"); fi
  if [[ -n "$PRUNE_LAMBDA" ]]; then args+=(--prune_lambda "$PRUNE_LAMBDA"); fi
  if [[ -n "$PRUNE_OPS" ]]; then
    # shellcheck disable=SC2206
    prune_ops_array=($PRUNE_OPS)
    args+=(--prune_ops "${prune_ops_array[@]}")
  fi
  if [[ "$BACKEND" == "hf" ]]; then
    args+=(--device "${DEVICE:-cuda:0}")
  else
    args+=(--tensor_parallel_size "$TENSOR_PARALLEL_SIZE" --gpu_memory_utilization "$GPU_MEMORY_UTILIZATION")
    args+=(--vllm_pruned_model_dir "$shard_output/vllm_pruned_model")
...
if [[ "$BACKEND" == "vllm" && "$DELETE_VLLM_PRUNED_MODEL" == "1" ]]; then
  find "$SHARD_ROOT" -maxdepth 2 \( -name 'vllm_pruned_model' -o -name 'vllm_pruned_model.lock' \) -exec rm -rf {} +
fi

echo "8B sparse-vs-sparse alignment run complete: s50 scoring s20 generations."
