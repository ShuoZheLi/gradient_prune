#!/bin/bash
#SBATCH --job-name=qwen3_8b_self_align_s50
...
#SBATCH --output=slurm-%j_qwen3_8b_self_align_s50.out
#SBATCH --error=slurm-%j_qwen3_8b_self_align_s50.err

set -euo pipefail

# Multi-node dense-vs-pruned 8B self-alignment launcher.
# Scores dense 8B generations under a WANDA-pruned 8B model, sharding JSONL rows
...
# Runtime config
# -----------------------------
SPARSITY_LABEL="${SPARSITY_LABEL:-50}"
RUN_NAME="${RUN_NAME:-qwen3_8b_self_alignment_gap_sparsity_${SPARSITY_LABEL}}"
...
MODEL_ID="${MODEL_ID:-qwen3_8b_wanda_s0.50_alignment_gap}"
PRUNING_SPARSITY="${PRUNING_SPARSITY:-0.50}"
SCORE_ROOT="${SCORE_ROOT:-$RESULTS_BASE/qwen3_8b_wanda_math7500/scores}"
INPUT_JSONL="${INPUT_JSONL:-${DENSE_8B_GENERATIONS_JSONL:-}}"
if [[ -z "$INPUT_JSONL" ]]; then
  dense_run_search_root="$RESULTS_BASE/response_analysis/qwen3_8b_wanda_math500_response_analysis/runs"
  if [[ -d "$dense_run_search_root" ]]; then
    INPUT_JSONL="$(find "$dense_run_search_root" -path '*/dense/generations.jsonl' -type f -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR==1 {print $2}')"
  fi
fi
...
if [[ ! -f "$INPUT_JSONL" ]]; then
  ...
  echo "Set INPUT_JSONL=/path/to/dense_8b/generations.jsonl or DENSE_8B_GENERATIONS_JSONL=/path/to/dense_8b/generations.jsonl." >&2
  exit 1
fi
...
echo "Dense-vs-pruned 8B self-alignment run complete."
