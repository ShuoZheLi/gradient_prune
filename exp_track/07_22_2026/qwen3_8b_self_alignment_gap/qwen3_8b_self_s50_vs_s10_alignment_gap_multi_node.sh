#!/bin/bash
#SBATCH --job-name=qwen3_8b_s50_vs_s10
...
#SBATCH --output=slurm-%j_qwen3_8b_s50_vs_s10.out
#SBATCH --error=slurm-%j_qwen3_8b_s50_vs_s10.err

set -euo pipefail

# Multi-node sparse-vs-sparse 8B self-alignment launcher, scoring s10 generations under s50.
# Scores 8B WANDA s10 generations under the WANDA s50 8B model, sharding JSONL rows
...
TEACHER_SPARSITY_LABEL="${TEACHER_SPARSITY_LABEL:-10}"
...
MODEL_ID="${MODEL_ID:-qwen3_8b_wanda_s0.50_scoring_s0.10_generations}"
PRUNING_SPARSITY="${PRUNING_SPARSITY:-0.50}"
SCORE_ROOT="${SCORE_ROOT:-$RESULTS_BASE/qwen3_8b_wanda_math7500/scores}"
INPUT_JSONL="${INPUT_JSONL:-${SPARSE_8B_GENERATIONS_JSONL:-/scratch/09576/shuozhe/gradient_prune/results/response_analysis/qwen3_8b_wanda_response_analysis/runs/qwen3_8b_wanda_response_analysis_839835_20260717_181829/combined/generations.jsonl}}"
...
echo "8B sparse-vs-sparse alignment run complete: s50 scoring s10 generations."
