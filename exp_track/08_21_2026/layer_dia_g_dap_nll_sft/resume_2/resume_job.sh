#!/bin/bash
#SBATCH --job-name=qwen3_8b_layer_dia_g_resume2
...
#SBATCH --output=slurm-%j_qwen3_8b_layer_dia_g_resume2.out
#SBATCH --error=slurm-%j_qwen3_8b_layer_dia_g_resume2.err
...
mkdir -p "$LOG_DIR"
exec > >(tee -a "$LOG_DIR/resume2_${SLURM_JOB_ID:-manual}.log") \
  2> >(tee -a "$LOG_DIR/resume2_${SLURM_JOB_ID:-manual}.err" >&2)
...
RUN_ID="qwen3_8b_layer_dia_g_resume2_${SLURM_JOB_ID:-manual}"
...
echo "[layer_dia_g_dap_resume2] output_root=$OUTPUT_ROOT"
echo "[layer_dia_g_dap_resume2] score_dir=$SCORE_DIR"
echo "[layer_dia_g_dap_resume2] nodes=$NNODES world_size=$WORLD_SIZE timeout_minutes=$DISTRIBUTED_TIMEOUT_MINUTES"
...
echo "[layer_dia_g_dap_resume2] completed successfully: $SCORE_DIR/manifest.json"
