#!/bin/bash
#SBATCH --job-name=finish_dsk_r1_llama8b_deepscaler_n2
...
#SBATCH --time=2:00:00
#SBATCH --output=slurm-%j_finish_dsk_r1_llama8b_deepscaler_n2.out
#SBATCH --error=slurm-%j_finish_dsk_r1_llama8b_deepscaler_n2.err
...
# Resume/finalize collector for 2-response trajectories from the DeepSeek-R1-Distill-Llama-8B model
...
# -----------------------------
RUN_NAME="${RUN_NAME:-finish_deepseek_r1_distill_llama_8b_train_deepscaler_2_response}"
RUN_ID="${RUN_ID:-${RUN_NAME}_${SLURM_JOB_ID:-manual}}"

model_path="${MODEL_PATH:-${BASE_MODEL_PATH:-${MODEL_INIT_CKPT:-/work/09576/shuozhe/saved_model/DeepSeek-R1-Distill-Llama-8B}}}"
dataset_path="${DATASET_PATH:-/work2/09576/shuozhe/saved_dataset/MetaMathQA-math-500/train_deepscaler.parquet}"
output_dir="${OUTPUT_DIR:-$repo_root/saved_calibration_dataset/deepseek-r1-distill-llama-8b_train_deepscaler_correct_2_response}"
...
calib_parquet="${CALIB_PARQUET:-$output_dir/deepseek-r1-distill-llama-8b_train_deepscaler_correct_2_response.parquet}"
...
echo "[collect] standalone=1 deepseek_r1_distill_llama_8b=1"
