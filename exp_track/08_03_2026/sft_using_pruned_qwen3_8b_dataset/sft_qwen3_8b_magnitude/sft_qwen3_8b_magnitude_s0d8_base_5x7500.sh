#!/bin/bash
#SBATCH --job-name=sft_qwen3_8b_magnitude_s0d8
...
#SBATCH --output=sft_qwen3_8b_magnitude_s0d8_5x7500-%j.out
#SBATCH --error=sft_qwen3_8b_magnitude_s0d8_5x7500-%j.err
...
RUN_NAME="${RUN_NAME:-sft_qwen3_8b_magnitude_s0d8_base_5x7500}"
...
PRUNING_SPARSITY="${PRUNING_SPARSITY:-0.8}"
...
SPARSE_MASK_PATH="${SPARSE_MASK_PATH:-${RUN_DIR}/sparse_update_masks/magnitude_s${PRUNING_SPARSITY}.pt}"
...
# -----------------------------
# Defaults target Qwen3-8B magnitude-pruned sparse SFT at sparsity 0.8 on the full 5-response x 7500 math dataset.
...
# Sparse SFT: magnitude top-score entries are kept/trainable; all pruned entries are zeroed and stay zero.
...
build_sparse_update_mask() {
  if [[ "$sparse_update_enabled" != "true" && "$sparse_update_enabled" != "True" ]]; then
    echo "Sparse update disabled; skipping magnitude mask build."
    return 0
  fi

  if [[ "$DRY_RUN" == "1" ]]; then
    echo "DRY_RUN=1; skipping magnitude mask build. Planned mask path: $SPARSE_MASK_PATH"
    return 0
  fi
  ...
  if [[ ! -f "$SCORE_ROOT/metadata.json" ]]; then
    echo "Magnitude score root does not contain metadata.json: $SCORE_ROOT" >&2
    echo "Set SCORE_ROOT=/path/to/magnitude/scores, e.g. /scratch/09576/shuozhe/gradient_prune/results/qwen3_8b_magnitude_math7500/scores." >&2
    exit 3
  fi
