...
# Common evaluator for SFT Qwen3-8B FFN-WANDA s0d7 only-FFN-update lr 5e-5 checkpoints.
...
run_name="${RUN_NAME:-sft_qwen3_8b_ffn_wanda_s0d7_correct_5_response_only_ffn_update_lr_5e5_902625_${checkpoint_name}_math500_eval}"
...
results_subdir="${RESULTS_SUBDIR:-eval_distill/sft_qwen3_8b_ffn_wanda_s0d7_only_ffn_update_lr_5e5_902625/${checkpoint_name}}"
...
"$python_bin" - "$metrics_json" "$shard_dir" "$max_examples" "$num_responses_per_prompt" "$tokenizer_path" <<'PY_AGGREGATE_METRICS'
import sys
...
from eval_distill_metrics import write_eval_metrics
