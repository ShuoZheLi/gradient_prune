...
def main() -> None:
    parser = argparse.ArgumentParser(description="First-token diagnostics for WANDA + grad-norm rank-fusion experiment.")
    ...
    for row in eval_rows:
        ...
        if method == "wanda":
            mask_dir = args.result_root / "masks" / "method=wanda" / f"sparsity={sparsity}" / "lambda=none"
            label = f"wanda_s{sparsity}"
        elif method == METHOD:
            mask_dir = args.result_root / "masks" / f"method={METHOD}" / f"sparsity={sparsity}" / f"lambda={lambda_value}"
            label = f"fusion_s{sparsity}_lambda{lambda_value}"
        else:
            continue
